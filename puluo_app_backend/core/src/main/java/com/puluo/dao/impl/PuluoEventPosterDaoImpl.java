...
public class PuluoEventPosterDaoImpl extends DalTemplate implements
	...
	public boolean createTable() {
		try {
			String createSQL = new StringBuilder().append("create table ")
					...
					.append("image_url text, ").append("thumbnail text, ")
					.append("event_info_uuid text, ")
					.append("poster_type text, ").append("poster_rank int, ")
		...
		try {
			...
			if (resCnt == 0) {
				updateSQL = new StringBuilder()
						.append("insert into ")
						.append(super.getFullTableName())
						.append(" (event_poster_uuid, image_url, event_info_uuid,poster_type,poster_rank, created_at)")
						.append(" values ('" + photo.imageId() + "', ")
						.append(Strs.isEmpty(photo.imageName()) ? "null" : "'"
								+ photo.imageName() + "'")
						.append(", ")
						.append(Strs.isEmpty(photo.eventInfoUUID()) ? "null"
								: "'" + photo.eventInfoUUID() + "'")
						.append(", ")
						.append("'")
						.append(photo.posterType().name())
						.append("', ")
						.append(photo.rank())
						.append(", ")
						.append(Strs.isEmpty(TimeUtils.formatDate(photo
								.createdAt())) ? "now()::timestamp" : "'"
								+ TimeUtils.formatDate(photo.createdAt()) + "'")
						.append(")").toString();
				log.info(Strs.join("SQL:", updateSQL));
				getWriter().update(updateSQL);
				return true;
			} else {
				throw new PuluoDatabaseException("event_poster_uuid为'"
						+ photo.imageId() + "'已存在不能插入数据！");
			}
		} catch (Exception e) {
	...
	@Override
	public List<PuluoEventPoster> getEventPosterByInfoUUID(
			String event_info_uuid) {
		SqlReader reader = getReader();
		StringBuilder selectSQL = new StringBuilder()
				.append("select * from ")
				.append(super.getFullTableName())
				.append(" where event_info_uuid = ? order by poster_rank");
		log.info(selectSQL);
		List<PuluoEventPoster> entities = reader.query(selectSQL.toString(),
				new Object[] { event_info_uuid }, new PuluoEventPosterMapper());
		return entities;
	}
		...
		try {
			...
			if (reader.queryForInt(querySQL) > 0) {
				...
				if (!Strs.isEmpty(photo.eventInfoUUID())) {
					updateSQL.append(" event_info_uuid = '"
							+ photo.eventInfoUUID() + "',");
					comma = true;
				}
				if (photo.posterType() != null) {
					updateSQL.append(" poster_type = '"
							+ photo.posterType().name() + "',");
					comma = true;
				}
				if (photo.rank() > 0) {
					updateSQL.append(" poster_rank = " + photo.rank() + ",");
					comma = true;
				}
				if (comma) {
					updateSQL.deleteCharAt(updateSQL.length() - 1);
				}
				updateSQL.append(" where event_poster_uuid = '"
						+ photo.imageId() + "'");
	...
	public PuluoEventPoster getEventPosterByUUID(String uuid) {
		...
		StringBuilder selectSQL = new StringBuilder().append("select * from ")
				.append(super.getFullTableName())
				.append(" where event_poster_uuid = ?");
		List<PuluoEventPoster> entities = reader.query(selectSQL.toString(),
				new Object[] { uuid }, new PuluoEventPosterMapper());
	...
	public boolean updatePosterRank(String poster_uuid, int rank) {
		try {
			...
			String querySQL = new StringBuilder()
					.append("select count(1) from ")
					.append(super.getFullTableName())
					.append(" where event_poster_uuid = '" + poster_uuid + "'")
					.toString();
			log.info(reader.queryForInt(querySQL));
			StringBuilder updateSQL;
			if (reader.queryForInt(querySQL) > 0) {
				updateSQL = new StringBuilder()
						.append("update ")
						.append(super.getFullTableName())
						.append(" set poster_rank = ")
						.append(rank)
						.append(" where event_poster_uuid = '" + poster_uuid
								+ "'");
				log.info(Strs.join("SQL:", updateSQL.toString()));
				int updatedCnt = getWriter().update(updateSQL.toString());
				return updatedCnt > 0;
			} else {
				throw new PuluoDatabaseException("event_poster_uuid为'"
						+ poster_uuid + "'不存在不能更新数据！");
			}
		} catch (Exception e) {
			log.info(e.getMessage());
			return false;
...
class PuluoEventPosterMapper implements RowMapper<PuluoEventPoster> {
	...
	public PuluoEventPoster mapRow(ResultSet rs, int rowNum)
		...
		PuluoEventPosterImpl eventPoster = new PuluoEventPosterImpl(
				rs.getString("event_poster_uuid"), rs.getString("image_url"),
