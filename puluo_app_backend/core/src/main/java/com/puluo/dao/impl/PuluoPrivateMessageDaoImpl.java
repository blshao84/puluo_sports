...
public class PuluoPrivateMessageDaoImpl extends DalTemplate implements
	...
	public PuluoPrivateMessage getFriendRequestMessage(String userUUID) {
		try {
			...
			StringBuilder selectSQL = new StringBuilder().append("select * from ")
					.append(super.getFullTableName())
					.append(" where to_user_uuid = ? and message_type = '" + PuluoMessageType.FriendRequest.name() + "'" );
			...
			if (entities.size() == 1)
				...
				throw new PuluoDatabaseException("通过userUUID查到多个FriendRequestMessage！");
			else
				return null;
		} catch (Exception e) {
			log.error(e.getMessage());
	...
	public PuluoPrivateMessage[] getMessagesByUser(String userUUID,
		...
		try {
			...
			StringBuilder selectSQL = new StringBuilder().append("select * from ")
					.append(super.getFullTableName())
					.append(" where (from_user_uuid = ? or to_user_uuid = ?) and message_type <> '" + PuluoMessageType.FriendRequest.name() + "'" );
			...
			log.info(selectSQL.toString());
			List<PuluoPrivateMessage> entities = reader.query(selectSQL.toString(), new Object[]{userUUID, userUUID},
					new RowMapper<PuluoPrivateMessage>() {
						@Override
						public PuluoPrivateMessage mapRow(ResultSet rs, int rowNum)
								throws SQLException {
