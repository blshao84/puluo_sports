...
public class PuluoEventMemoryDaoImpl extends DalTemplate implements
		PuluoEventMemoryDao {
	...
	public boolean createTable() {
		try {
			String createSQL = new StringBuilder().append("create table ")
					...
					.append("event_uuid text, ").append("user_uuid text, ")
					.append("timeline_uuid text)").toString();
			log.info(createSQL);
			...
			String indexSQL2 = new StringBuilder()
					.append("create index " + super.getFullTableName()
							+ "_i_memory_uuid on ")
					.append(super.getFullTableName()).append(" (memory_uuid)")
					.toString();
			log.info(indexSQL2);
	...
	public List<PuluoEventMemory> getEventMemoryByEventUUID(String event_uuid) {
		...
		List<PuluoEventMemory> entities = reader.query(selectSQL,
				new Object[] { event_uuid }, new PuluoEventMemoryMapper());
		return entities;
	}
		...
		try {
			...
			if (resCnt == 0) {
				updateSQL = new StringBuilder()
						...
						.append(Strs.isEmpty(memory.imageName()) ? "null" : "'"
								+ memory.imageName() + "'")
						.append(", ")
						.append(Strs.isEmpty(memory.eventId()) ? "null" : "'"
								+ memory.eventId() + "'")
						.append(", ")
						.append(Strs.isEmpty(memory.userId()) ? "null" : "'"
								+ memory.userId() + "'")
						.append(", ")
						.append(Strs.isEmpty(memory.timelineId()) ? "null"
								: "'" + memory.timelineId() + "'").append(")")
						.toString();
				log.info(Strs.join("SQL:", updateSQL));
				getWriter().update(updateSQL);
				return true;
			} else {
				throw new PuluoDatabaseException("memory_uuid为'"
						+ memory.imageId() + "'已存在不能插入数据！");
			}
		} catch (Exception e) {
			log.info(e.getMessage());
			return false;
	...
	public PuluoEventMemory getEventMemoryByUUID(String uuid) {
		...
		StringBuilder selectSQL = new StringBuilder().append("select * from ")
				.append(super.getFullTableName())
				.append(" where memory_uuid = ?");
		List<PuluoEventMemory> entities = reader.query(selectSQL.toString(),
				new Object[] { uuid }, new PuluoEventMemoryMapper());
		return verifyUniqueResult(entities);
	}

	public boolean deleteByEventUUID(String event_uuid) {
		return super.deleteByUniqueKey("event_uuid", event_uuid);
...
class PuluoEventMemoryMapper implements RowMapper<PuluoEventMemory> {
	...
	public PuluoEventMemory mapRow(ResultSet rs, int rowNum)
		...
		PuluoEventMemoryImpl eventPoster = new PuluoEventMemoryImpl(
				rs.getString("memory_uuid"), rs.getString("image_url"),
				rs.getString("event_uuid"), rs.getString("user_uuid"),
