...
public abstract class Strs {
	 ...
	 * @return
	 */
	public static String join(Collection collection, String split, String left,
			String right) {
	...
	public static String join(Map map, String entrySplit, String kvSplit) {
		...
		{
			builder.append(entry.getKey()).append(kvSplit)
					.append(entry.getValue());
			builder.append(entrySplit);
		}
		if (!Strs.isEmpty(entrySplit)) {
			builder.delete(builder.length() - entrySplit.length(),
					builder.length());
		}
		return builder.toString();
	...
	public static void main(String[] args) {
		System.out.println(Strs
				.filter(" dsdfsdf,23.990901023123{}[]]123123(**@&^@#!@J吴津经"));
	}
