package com.unuotech.payment.unionpay.utils;

import com.unuotech.payment.config.UnionPayConf;
import com.unuotech.payment.example.UnionPayQuery;
...
public class Sms {
	...
	public void send() {
		...
		String res = quickPayUtils.doPostQueryCmd(UnionPayConf.smsUrl,new UnionPayUtils().createBackStrForBackTrans(valueVo, UnionPayConf.smsVo));
		
		if (res != null && !"".equals(res)) {
			
			String[] arr = UnionPayUtils.getResArr(res);
			
			if(new UnionPayQuery().checkSecurity(arr)){//验证签名
