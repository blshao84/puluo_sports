...
public class UserSearchFunctionalTest extends APIFunctionalTest {
	...
	private void testSearch1Parameter(String name, String key, String value, int size) {
		log.info(name + " start!");
		try {
			login();
			...
			for (int i=0; i<details.getArray().length(); i++) {
				String public_info = super.getStringFromJson(new JsonNode(String.valueOf(details.getArray().get(i))), "public_info");
				String result = super.getStringFromJson(new JsonNode(public_info), key);
	...
	private void testSearch2Parameters(String name, String[] key, String[] value, int size) {
		log.info(name + " start!");
		try {
			login();
			Assert.assertFalse("successful login should give not null session id", Strs.isEmpty(sessionId));
			
			String str = String.format("{\"token\":\"%s\", \"" + key[0] + "\":\"%s\", \"" + key[1] + "\":\"%s\"}", sessionId, value[0], value[1]);
			JsonNode json = callAPI("users/search", str);
			log.info(json);
			JsonNode details = new JsonNode(super.getStringFromJson(json, "details"));
			log.info(details.getArray().length());
	...
	private void testSearch3Parameters(String name, String[] key, String[] value, int size) {
		log.info(name + " start!");
		try {
			login();
			Assert.assertFalse("successful login should give not null session id", Strs.isEmpty(sessionId));
			
			String str = String.format("{\"token\":\"%s\", \"" + key[0] + "\":\"%s\", \"" + key[1] + "\":\"%s\", \"" + key[2] + "\":\"%s\"}", sessionId, value[0], value[1], value[2]);
			JsonNode json = callAPI("users/search", str);
			log.info(json);
			JsonNode details = new JsonNode(super.getStringFromJson(json, "details"));
			log.info(details.getArray().length());
	...
	private void testSearch4Parameters(String name, String[] key, String[] value, int size) {
		log.info(name + " start!");
		try {
			login();
			Assert.assertFalse("successful login should give not null session id", Strs.isEmpty(sessionId));
			
			String str = String.format("{\"token\":\"%s\", \"" + key[0] + "\":\"%s\", \"" + key[1] + "\":\"%s\", \"" + key[2] + "\":\"%s\", \"" + key[3] + "\":\"%s\"}", sessionId, value[0], value[1], value[2], value[3]);
			JsonNode json = callAPI("users/search", str);
			log.info(json);
			JsonNode details = new JsonNode(super.getStringFromJson(json, "details"));
			log.info(details.getArray().length());
	...
	public void testSearchByMobile() {
		testSearch1Parameter("testSearchByMobile", "mobile", "123456789", 1);
	}
