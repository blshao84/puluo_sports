...
public class ApproveFriendFunctionalTest extends APIFunctionalTest {
	public static Log log = LogFactory.getLog(ApproveFriendFunctionalTest.class);
	...
	public void testApproveFriendFromFriend(){
		super.runAuthenticatedTest(new FriendRequestFunctionalTestRunner() {
			...
			public void run(String session) throws UnirestException {
				JsonNode json = callAPI("/users/friends/request/approve", inputs(session));
				log.info(json);
				PuluoUserFriendshipDao friendshipdao = DaoApi.getInstance().friendshipDao();
				Assert.assertTrue("they should be friends", friendshipdao.isFriend(uuid1, uuid2));
				PuluoFriendRequestDao friendRequestDao = DaoApi.getInstance().friendRequestDao();
				List<PuluoFriendRequest> requests = friendRequestDao.getFriendRequestByUsers(uuid1,uuid2,FriendRequestStatus.Approved);
				PuluoFriendRequest request =  requests.get(0);
				requests.addAll(friendRequestDao.getFriendRequestByUsers(uuid2,uuid1,FriendRequestStatus.Approved));
